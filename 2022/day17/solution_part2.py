...
def progress_piece(board: Board, jets: Jets, cycles: set, printme: bool = False):
    x = (board.piece_index, jets.i)
    if x in cycles and x[0] == 0:
        print(f'Cycle detected after {board.piece_count} pieces added: piece={x[0]}, jet={x[1]} - height={board.highest_occupied()+1}')
    else:
        cycles.add(x)
...
if __name__ == "__main__":
    ...
    with open("data.csv", 'w') as fl:
        for x in range(500000):
            progress_piece(board, jets, cycler, printme = False)
            y = board.highest_occupied() + 1
            fl.write(f'{x+1},{y}\n')

    # Detected a cycle:
    # Every 1735 pieces we add 2673 to the height
    # This cycle begins at piece 1875 when the height is already 2876
    # Piece 1000000000000 will drop exactly 576368875 full cycles
    # after piece 1875, so its height will be:
    # 2876 + n_full_cycles * 2673 = 1540634005751
